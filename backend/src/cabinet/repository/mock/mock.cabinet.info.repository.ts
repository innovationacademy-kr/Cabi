...
export class MockCabinetInfoRepository implements ICabinetInfoRepository {
    ...
    async getCabinetInfo(cabinet_id: number): Promise<CabinetDto> {
        const ret = this.MockCabinetInfoEntity
        .find(target => (target.cabinet_id === cabinet_id));
        return ret;
    }
    ...
    async updateCabinetStatus(
        ...
        this.MockCabinetInfoEntity
        .find(cabinetData => (cabinetData.cabinet_id === cabinet_id))
        ['cabinet_status'] = status;
    }

    async getLentUsers(cabinet_id: number): Promise<LentDto[]> {
        const ret = this.MockLentEntity
        .filter(lentData => (lentData.cabinet_id === cabinet_id))
        .map((target): LentDto => ({
