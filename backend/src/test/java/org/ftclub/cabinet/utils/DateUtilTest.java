...
class DateUtilTest {
    ...
    @Test
    public void 연체되기_하루_전날() {
        Calendar cal = Calendar.getInstance();
        cal.set(2023, Calendar.JANUARY, 2, 00, 00);
        Date now = cal.getTime();
        cal.set(2023, Calendar.JANUARY, 3, 23, 59);
        Date expiredAt = cal.getTime();
        assertEquals(-1, DateUtil.calculateTwoDateDiffCeil(now, expiredAt));
    }

    @Test
    public void 연체되기_전_마지막_날() {
        Calendar cal = Calendar.getInstance();
        cal.set(2023, Calendar.JANUARY, 2, 00, 00);
        Date now = cal.getTime();
        cal.set(2023, Calendar.JANUARY, 2, 23, 59);
        Date expiredAt = cal.getTime();
        assertEquals(0, DateUtil.calculateTwoDateDiffCeil(now, expiredAt));
    }
    ...
    public void 연체_당일() {
        ...
        cal.set(2023, Calendar.JANUARY, 2, 00, 00);
        Date now = cal.getTime();
        cal.set(2023, Calendar.JANUARY, 1, 23, 59);
        Date expiredAt = cal.getTime();
        assertEquals(1, DateUtil.calculateTwoDateDiffCeil(now, expiredAt));
    }
}