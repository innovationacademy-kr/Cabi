...
public class EmailAlarmSender {
	...
	void send(User user, AlarmEvent alarmEvent) {
		...
		try {
			emailService.sendMail(user.getName(), user.getEmail(), mailDto.getSubject(),
	...
	private MailDto getMailDto(Alarm alarm) {
		if (alarm instanceof LentSuccessAlarm) {
			return new MailDto(alarmProperties.getLentSuccessSubject(),
					alarmProperties.getLentSuccessMailTemplateUrl());
		}
			...
			return new MailDto(alarmProperties.getExtensionIssuanceSubject(),
					alarmProperties.getExtensionIssuanceMailTemplateUrl());
		}
