...
then
	...
	# Create database and grant all on $MARIADB_USER
	echo "Create database and grant all on $MARIADB_USER"
	echo -e "${GREEN} DROP DATABASE IF EXISTS $MARIADB_DATABASE_V2; CREATE DATABASE $MARIADB_DATABASE_V2; GRANT ALL ON $MARIADB_DATABASE_V2.* TO '$MARIADB_USER'@'%' IDENTIFIED BY '$MARIADB_PASSWORD'; FLUSH PRIVILEGES; ${RESET}"
	echo "DROP DATABASE IF EXISTS $MARIADB_DATABASE_V2; CREATE DATABASE $MARIADB_DATABASE_V2; GRANT ALL ON $MARIADB_DATABASE_V2.* TO '$MARIADB_USER'@'%' IDENTIFIED BY '$MARIADB_PASSWORD'; FLUSH PRIVILEGES;" | mysql -u$MARIADB_USER -p$MARIADB_PASSWORD

	# Import database
	echo "Import database"
	echo -e "${GREEN} mysql -u$MARIADB_USER -p$MARIADB_PASSWORD $MARIADB_DATABASE_V2 < /database/42cabi_v2_test.sql ${RESET}"
