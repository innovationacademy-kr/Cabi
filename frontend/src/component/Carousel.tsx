...
export default function Carousel(props:any){
  const cabinetBlock = (f_idx:number, s_idx:number) => {
    ...
    for (let i = 0; i < cab.length; i++){
      ...
      list.push(
        <CabinetBox className="d-block w-100" setTarget={props.setTarget} key={`cab_box_${cab[i].cabinet_id}`} cabinet_id={cab[i].cabinet_num} intra_id={id ? id.intra_id : ""}></CabinetBox>
      );
    }
    return list;
  }
