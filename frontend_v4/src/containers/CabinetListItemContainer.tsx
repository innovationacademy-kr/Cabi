...
const CabinetListItemContainer = (props: CabinetInfo) : JSX.Element=> {
    ...
    if (props.status !== "BANNED") //사용불가가 아닌 모든 경우
    {
...
const CabinetListItemStyled = styled.div<{status: CabinetStatus, cabinet_id: number}>`
    position: relative;    
    background-color: ${(props) => cabinetStatusColorMap[props.status]};
    ${(props) =>
        props.cabinet_id === MY_INFO.cabinet_id &&
        css`background-color: var(--mine);`
    }
    width: 80px;
    height: 80px;
    margin: 5px;
...
const CabinetLabelStyled = styled.p<{status: CabinetStatus, cabinet_id:number}>`
    position: absolute;
    top: 44px;
    left: 10px;
    font-size: 14px;
...
const CabinetNumberStyled = styled.p<{status: CabinetStatus, cabinet_id: number}>`
    position: absolute;
    top: 10px;
    right: 10px;
    font-size: 14px;
